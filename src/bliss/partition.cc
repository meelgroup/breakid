...
void Partition::goto_backtrack_point(BacktrackPoint p)
{
    ...
    while (refinement_stack.size() > dest_refinement_stack_size) {
        ...
        if (i.prev_nonsingleton_first >= 0) {
            Cell* const prev_cell =
                get_cell(elements[i.prev_nonsingleton_first]);
        ...
        if (i.next_nonsingleton_first >= 0) {
            Cell* const next_cell =
                get_cell(elements[i.next_nonsingleton_first]);
...
Partition::Cell* Partition::aux_split_in_two(Partition::Cell* const cell,
                                             const unsigned int first_half_size)
{
    ...
    if (cell->is_unit()) {
        if (cell->prev_nonsingleton)
            cell->prev_nonsingleton->next_nonsingleton =
                cell->next_nonsingleton;
        else
            first_nonsingleton_cell = cell->next_nonsingleton;
        if (cell->next_nonsingleton)
            cell->next_nonsingleton->prev_nonsingleton =
...
size_t Partition::print_signature(FILE* const fp, const bool add_newline) const
{
    size_t r = 0;
    const char* cell_sep = "";
    r += fprintf(fp, "[");
 ...
 */
Partition::Cell* Partition::sort_and_split_cell255(Partition::Cell* const cell,
...
unsigned int Partition::cr_split_level(
    const unsigned int level, const std::vector<unsigned int>& splitted_cells)
{
    assert(cr_enabled);
