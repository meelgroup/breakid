...
std::vector<bool>& AbstractGraph::long_prune_allocget_fixed(
...
std::vector<bool>& AbstractGraph::long_prune_allocget_mcrs(
    const unsigned int index)
{
    const unsigned int i = index % long_prune_max_stored_autss;
...
void AbstractGraph::search(const bool canonical, Stats& stats)
{
   ...
   */
    {
        TreeNode root;
        Partition::Cell* split_cell =
            find_next_cell_to_be_splitted(p.first_cell);
   ...
   */
    while (!search_stack.empty()
        ...
        (*max_num_steps)--;
        if (verbstr && verbose_level >= 2 && (*max_num_steps & 0xfff) == 0xfff) {
       ...
       */
        if (opt_use_long_prune && current_level >= 1 && !current_node.fp_on) {
            unsigned int begin =
                (current_node.long_prune_begin > long_prune_begin)
...
#endif
                if (fixed[search_stack[search_stack.size() - 1 - 1]
                              .split_element] == false) {
...
#endif
                    assert(
                        fixed[search_stack[current_level - 1].split_element] ==
                        true);
       ...
       */
        {
        	 ...
        	 */
                    if (index == cell->length &&
                        all_same_level == current_level + 1)
                        all_same_level = current_level;
                    if (verbstr && verbose_level >= 2) {
                        fprintf(verbstr,
                                "Level %u: orbits=%u, index=%u/%u, "
                                "all_same_level=%u\n",
                                current_level, first_path_orbits.nof_orbits(),
                                index, cell->length, all_same_level);
                        fflush(verbstr);
                    }
                }
                continue;
       ...
       */
        ...
        if (!first_path_info.empty()) {
            ...
            if (refine_equal_to_first) {
                ...
                if (subcertificate_length !=
                    first_pinfo.subcertificate_length) {
                    refine_equal_to_first = false;
                    if (opt_use_failure_recording)
            ...
            if (canonical && (refine_cmp_to_best == 0)) {
                ...
                if (subcertificate_length < bestp_info.subcertificate_length) {
                    refine_cmp_to_best = -1;
                } else if (subcertificate_length >
                           bestp_info.subcertificate_length) {
                    refine_cmp_to_best = 1;
                ...
                if (current_node.fp_on)
                    ...
                    for (unsigned int i = current_level; i > 0; i--) {
                        if (search_stack[i].fp_on)
                            break;
                        const FailureRecordingSet& s =
                            failure_recording_hashes[i];
                        if (i == current_level &&
        ...
        if (!is_equitable())
            fatal_error(
                "consistency check failed - partition after refinement is not "
                "equitable");
#endif
            ...
            if (opt_use_comprec) {
                ...
                if (p.nof_discrete_cells() ==
                    ...
                    if (first_path_info.empty() or child_node.fp_cert_equal) {
                        if (cep.first_checked == false) {
                            ...
                            TreeNode& old_info =
                                search_stack[cep.creation_level];
        		 ...
        		 * first path automorphisms */
        		   ...
        		   * Thus we can only backtrack to the previous level */
        		       ...
        		       */
                                    for (unsigned int i = 0; i < N; i++) {
                                        if (p.get_cell(p.elements[i])
                                                ->is_unit()) {
                                            best_path_labeling[p.elements[i]] =
                                                i;
                                            best_path_labeling_inv[i] =
                                                p.elements[i];
                                        }
                                    }
                                    ...
                                    unsigned int postfix_start =
                                        cep.creation_level;
                                    assert(postfix_start <
                                           best_path_info.size());
                                    while (
                                        p.get_cell(best_path_info[postfix_start]
                                                       .splitting_element)
                                            ->is_unit()) {
                                        postfix_start++;
                                        assert(postfix_start <
                                    ...
                                    unsigned int postfix_start_cert =
                                        best_path_info[postfix_start]
                                            .certificate_index;
                                    std::vector<PathInfo> best_path_temp =
                                        best_path_info;
                                    best_path_info.clear();
                                    for (unsigned int i = 0;
                                        ...
                                        bp_info.splitting_element =
                                            ss_info.split_element;
                                        bp_info.certificate_index =
                                            ss_info.certificate_index;
                                    ...
                                    for (unsigned int i = postfix_start;
                                        ...
                                        best_path_info.push_back(
                                            best_path_temp[i]);
                                        best_path_info[best_path_info.size() -
                                                       1]
                                            .certificate_index =
                                            best_path_info
                                                [best_path_info.size() - 2]
                                                    .certificate_index +
                                            best_path_info
                                                [best_path_info.size() - 2]
                                                    .subcertificate_length;
                                    ...
                                    std::vector<unsigned int>
                                        certificate_best_path_old =
                                            certificate_best_path;
                                    certificate_best_path =
                                        certificate_current_path;
               ...
               * new non-uniformity subcomponents */
                //if(nucr_find_first_component(cr_level) == true &&
                // p.nof_discrete_cells() + cr_component_elements <
                // cr_cep_stack[cr_cep_index].discrete_cell_limit)
         ...
         */
             ...
             * Set the image of all elements in non-disrete cells accordingly */
                for (Partition::Cell* c = p.first_nonsingleton_cell; c;
                    ...
                    for (unsigned int i = c->first; i < c->first + c->length;
                         i++)
                        if (p.get_cell(
                            ...
                            best_path_automorphism[p.elements[i]] =
                                p.elements[i];
                }
            }
            ...
            for (unsigned int i = search_stack.size(); i > 0; i--) {
                if ((int)first_path_info[gca_level_with_first]
                        .splitting_element !=
            ...
            for (unsigned int i = search_stack.size(); i > 0; i--) {
                if ((int)best_path_info[gca_level_with_best]
                        .splitting_element !=
...
bool Digraph::split_neighbourhood_of_cell(Partition::Cell* const cell)
{
    ...
    while (!neighbour_heap.is_empty()) {
        ...
        Partition::Cell* const last_new_cell =
            p.zplit_cell(neighbour_cell, true);
